...
public class SettlementsCreateHeldTransferIntentFunction
{
    private readonly IOrderWorkflowService _workflow;

    public SettlementsCreateHeldTransferIntentFunction(IOrderWorkflowService workflow)
    {
        _workflow = workflow;
    }

    [Function("settlements-create-held-transfer-intent")]
    public async Task<HttpResponseData> RunAsync(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "settlements/held-transfer")] HttpRequestData req,
        FunctionContext executionContext)
    {
        ...
        var order = await _workflow.CreateHeldTransferAsync(payload.OrderId, payload.Reason ?? "manual request", cancellationToken);
        if (order is null)
        {
