...
public class DealerPoAckHandlerFunction
{
    private readonly IOrderWorkflowService _workflow;
    private readonly ISignatureService _signatures;
    private readonly WorkflowOptions _options;

    public DealerPoAckHandlerFunction(IOrderWorkflowService workflow, ISignatureService signatures, IOptions<WorkflowOptions> options)
    {
        _workflow = workflow;
    ...
    [Function("dealer-po-ack-handler")]
    public async Task<HttpResponseData> RunAsync(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "dealer/ack")] HttpRequestData req,
        FunctionContext executionContext)
    {
        ...
        var order = await _workflow.HandleDealerResponseAsync(payload.Token, payload.Approved, payload.Actor, cancellationToken);
        if (order is null)
        {
